# src/evaluation/calculate_metrics_phi4.py
# Phi-4 model evaluation
...
<|assistant|>"""
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # Clean Phi-4 special tokens if any
            raw = raw.replace("<|end|>",       "").strip()
            raw = raw.replace("<|assistant|>", "").strip()
            raw = raw.replace("<|user|>",      "").strip()
            raw = raw.replace("<|system|>",    "").strip()
...
def evaluate_single(
    ...
    answer  = generate_answer_phi4(question, context)
    ...
    return {
        ...
        "by_category"   : {
            c: round(float(np.mean([
                ...
                for r in results if r["category"] == c
            ])), 2)
            for c in set(r["category"] for r in results)
        },
        "by_difficulty" : {
            d: round(float(np.mean([
                r["scope"]["weighted_total"]
                for r in results if r["difficulty"] == d
            ])), 2)
...
def print_report(summary: dict):
    ...
    path = f"results/eval_PHI4_{ts}.json"
...
if __name__ == "__main__":
    ...
    print(f"\nEvaluating {len(qa_data)} with {LLM_MODEL.upper()}...")
    print("="*60)
