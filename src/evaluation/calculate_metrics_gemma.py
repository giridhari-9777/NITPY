# src/evaluation/calculate_metrics_gemma.py
# Gemma3 model evaluation
...
def get_chunks(
    ...
    stopwords = {
        ...
        "at","to","of","and","or","but","for","with","by",
        "what","how","why","when","which","who","do","does"
    }
    keywords = [
        t for t in q_tokens
...
def generate_answer_gemma(
    ...
    # Gemma uses clean instruction format
...
def evaluate_single(
    ...
    answer  = generate_answer_gemma(question, context)
...
def aggregate(results: list) -> dict:
    ...
    def avg(key, section):
        return float(np.mean([r[section][key] for r in results]))
    ...
    return {
        ...
        "avg_confidence"      : round(float(np.mean([
            r["faith_rel"]["answer_relevance"] for r in results
        ])), 3),
...
def _by_cat(results):
    ...
    return {c: round(float(np.mean(s)),2) for c,s in by_cat.items()}
...
def _by_diff(results):
    ...
    return {d: round(float(np.mean(s)),2) for d,s in by_diff.items()}
...
def print_report(summary: dict):
    ...
    path = f"results/eval_GEMMA_{ts}.json"
...
if __name__ == "__main__":
    ...
    try:
        ...
        if test.status_code == 200:
            ...
            print(f"  ❌ {LLM_MODEL} not responding")
