...
def start_new_chat():
    ...
    st.session_state.quick_q       = None
    st.session_state.page          = "chat"
    st.session_state.save_msg      = "✅ Previous chat saved!"
...
def process_question(
    ...
    if is_edit and edit_idx is not None:
        st.session_state.messages      = (
            st.session_state.messages[:edit_idx]
        )
    ...
    with st.spinner("🩺 Analyzing..."):
        try:
            ...
            if qtype != "non_medical":
                ...
                if not result.get("hallucination", {}).get(
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if st.button(
            ...
            if fname:
                st.session_state.save_msg = f"✅ Saved!"
                st.rerun()
            else:
        ...
        if st.session_state.last_cancer:
            st.markdown(
                f"<div class='mem-badge'>"
                f"🎯 {st.session_state.last_cancer}"
                f"</div>",
                unsafe_allow_html=True
            )
        else:
        ...
        if st.session_state.all_scores:
            avg = round(
                ...
                len(st.session_state.all_scores), 2
            )
            st.markdown("**📊 Avg Score**")
            color = (
                "normal"  if avg >= 4.4 else
                "off"     if avg >= 3.5 else
                "inverse"
            )
            st.metric(
                "score_metric", f"{avg} / 5.0",
                delta       = "✅ Target!" if avg >= 4.4 else "Target: 4.4",
                delta_color = color,
                label_visibility = "collapsed"
            )
        ...
        if st.session_state.page == "chat":
            ...
            quick_qs = [
                ...
                "What is stage 4 cancer prognosis?",
        ...
        if role == "patient":
            ...
            if is_editing:
                ...
                with cm:
                    if is_edit:
                        st.markdown(
                            "<div class='edited-tag'>✏️ Edited</div>",
                            unsafe_allow_html=True
                        )
                    st.markdown(
                        f"<div class='patient-msg'>"
                        f"<div class='patient-label'>"
                        f"👤 {st.session_state.patient_name}</div>"
                        f"<div class='patient-text'>"
...
def _render_meta(meta: dict):
    ...
    if meta.get("is_followup"):
        st.info("🔄 Doctor is asking a clarifying question.")
        return
    ...
    with st.expander("📋 Response Details", expanded=False):
        ...
        with c1:
            if score >= 4.0:
                st.markdown(
                    f"<div class='score-good'>✅ {score}/5</div>",
                    unsafe_allow_html=True
                )
            elif score >= 3.0:
                st.markdown(
                    f"<div class='score-warn'>⚠️ {score}/5</div>",
                    unsafe_allow_html=True
                )
            else:
                st.markdown(
                    f"<div class='score-bad'>❌ {score}/5</div>",
                    unsafe_allow_html=True
                )
        with c2:
            st.metric(
                "Type",
                meta.get("question_type", "?").upper()[:10]
            )
        with c3:
        ...
        if resolved and resolved != original:
            st.markdown(
                f"<span style='background:#1e2a1e;border:1px solid "
                f"#4caf50;border-radius:5px;padding:3px 8px;"
                f"font-size:12px;color:#99ff99;'>"
                f"🧠 Understood as: {resolved}</span>",
                unsafe_allow_html=True
            )

        if cancer and cancer not in ["cancer", ""]:
            st.markdown(
                f"<div class='mem-badge'>🎯 {cancer}</div>",
                unsafe_allow_html=True
            )
        ...
        if hall:
            ...
            with b1: st.metric("Faithfulness", hall.get("faithfulness", 0.0))
            with b2: st.metric("Relevance",    hall.get("relevance",    0.0))
            with b3: st.metric("Safety",       hall.get("safety",       "LOW"))
...
def page_chat(agent):
    ...
    if not st.session_state.messages:
        st.markdown(
            ...
            <b>🩺 Doctor AI:</b><br><br>
            <b>{greeting}!</b>
            {"Welcome, " + name + "!" if name != "Patient" else "Welcome!"}<br><br>
            I know that cancer concerns can feel
            overwhelming — <b>you are not alone</b>.
            I am your dedicated oncology AI assistant,
            here to listen and support you.<br><br>
            I can help with:<br>
            ...
            <i>Please share what's on your mind.
            I am here for you.</i>
            </div>""",
            unsafe_allow_html=True
        )
    ...
    user_input = st.chat_input(
        "Ask about cancer... (Enter to send)"
    )
...
def page_history():
    ...
    st.markdown(
        f"**{len(filtered)} conversation(s)**"
        + (f" (filtered from {len(chats)})" if search else "")
    )
    st.divider()
    ...
    for chat in filtered:
        ...
        st.markdown(
            ...
            🕐 {saved_at} &nbsp;|&nbsp; 💬 {total} messages
            </span><br>
            <span style='color:#aaaacc;font-size:13px;'>
            {first_q}...
            </span></div>""",
            unsafe_allow_html=True
        )
        ...
        with cc:
            if st.button(
                ...
                # Load into live session
                st.session_state.messages      = msgs
                st.session_state.patient_name  = name
                st.session_state.session_id    = chat.get(
                    "session_id", _new_sid()
                )
                st.session_state.total_queries = len([
                    m for m in msgs if m.get("role") == "patient"
                ])
...
def page_view_chat():
    ...
    for msg in msgs:
        ...
        if role == "patient":
            st.markdown(
                f"<div class='patient-msg'>"
                f"<div class='patient-label'>👤 {name}</div>"
                f"<div class='patient-text'>{content}</div>"
                f"</div>",
                unsafe_allow_html=True
            )
...
def main():
    ...
    <div class='main-header'>
        ...
        <p>Powered by Agentic RAG + LLaMA3
        + 25 Oncology Textbooks</p>
        <p style='color:#888899;font-size:12px;'>
        ⚠️ For educational purposes only.
        Always consult a real oncologist.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with info_col:
        ...
        st.metric(
            "Passed",
            f"{passed}/{total}" if total > 0 else "0/0"
        )
        ...
        if st.session_state.last_cancer:
            st.markdown(
                f"<div class='mem-badge'>"
                f"🎯 {st.session_state.last_cancer}</div>",
                unsafe_allow_html=True
            )
            st.caption(
                "Ask follow-ups like:\n"
